...
assert_grep "install.sh copies lesson templates" "_lesson.md" "$TEAM_DIR/install.sh"
...
assert_grep "factseeker reads AGENTS.md for state path" "AGENTS.md.*project" "$KIL/agent/factseeker.md"
assert_grep "planer reads AGENTS.md for state path" "AGENTS.md.*project" "$KIL/agent/planer.md"
assert_grep "coder reads AGENTS.md for state path" "AGENTS.md.*project" "$KIL/agent/coder.md"
assert_grep "simplifer reads AGENTS.md for state path" "AGENTS.md.*project" "$KIL/agent/simplifer.md"
assert_grep "tester reads AGENTS.md for state path" "AGENTS.md.*project" "$KIL/agent/tester.md"
assert_grep "factseeker references project-state log" "project-state" "$KIL/agent/factseeker.md"
assert_grep "planer references project-state plan" "project-state.*planer_plan" "$KIL/agent/planer.md"
assert_grep "coder references project-state plan" "project-state.*planer_plan" "$KIL/agent/coder.md"

# --- Dry run install ---
echo ""
echo "[Install Dry Run]"
TMP_PROJECT="/tmp/test-agent-project-$$"
mkdir -p "$TMP_PROJECT"
TEAM_DIR="$TEAM_DIR" bash "$TEAM_DIR/install.sh" "test-project" "$TEAM_DIR" > /dev/null 2>&1 || true
if [ -d "$TEAM_DIR/projects/test-project" ]; then
    echo "  ✅ install.sh creates project state dir"
    PASS=$((PASS+1))
    assert "project has factseeker_log.md" test -f "$TEAM_DIR/projects/test-project/factseeker_log.md"
    assert "project has planer_plan.md" test -f "$TEAM_DIR/projects/test-project/planer_plan.md"
    assert "project has coder_log.md" test -f "$TEAM_DIR/projects/test-project/coder_log.md"
    assert "project has simplifer_log.md" test -f "$TEAM_DIR/projects/test-project/simplifer_log.md"
    assert "project has tester_log.md" test -f "$TEAM_DIR/projects/test-project/tester_log.md"
    assert "project has all 5 lesson files" test $(find "$TEAM_DIR/projects/test-project" -name "*_lesson.md" | wc -l) -eq 5
    # cleanup
    rm -rf "$TEAM_DIR/projects/test-project"
else
    echo "  ❌ install.sh creates project state dir"
    FAIL=$((FAIL+1))
fi
